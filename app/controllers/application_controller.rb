class ApplicationController < ActionController::Base
  ...
  def moneybird_error(exception)
    if exception.code == 401
      ...
      redirect_to projects_path, alert: 'Your Moneybird token is no longer valid, please re-authenticate.'
    else
      redirect_to projects_path, alert: "A moneybird error occurred: #{exception}"
    end
  end
end
